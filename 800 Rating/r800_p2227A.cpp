...
int main() {
    ...
    while(cs--) {
        ...
        if(!(left % 2) && !(right % 2)) cout << "YES" << endl; // 2 ta coordinates even a ase 
        else if(!(left % 2) || !(right % 2)) cout << "YES" << endl; // 1 ta coordinated even a ase
        else if(left % 2 && right % 2) cout << "NO" << endl; // 2 ta coordinated odd a ase
    }
